        ...
        jQuery.ajax({
            ...
            success: function (data) {
                if (data.status == 'success' && data.flag == 0) {
                    adsCount = visibleSections.length == 1 ? [0] : (visibleSections.length == 2 ? [0, 1] :(visibleSections.length >= 3 ? [0, 1, (visibleSections.length-1)] : []));
                    for(var i=0; i<adsCount.length; i++) {
                        jQuery(`.promotional-${visibleSections[adsCount[i]]}-${websiteId}`).removeClass('d-none');
                    }
                }
            },
            error() {
